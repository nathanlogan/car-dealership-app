...
class Inventory extends Component {
  ...
  getAllFilters() {
    ...
    this.cars.forEach((car) => {
      keys.forEach(
        (key) => !filters[key].includes(car[key]) && filters[key].push(car[key])
      );
    });
    ...
    filters.forEach((filter) => {
      if (filter.type === "select") {
        results = results.filter((car) =>
          filter.values.includes(car[filter.facet])
        );
      } else {
        console.log("filtering range");
        results = results.filter(
          (car) =>
            car[filter.facet] >= filter.values[0] &&
            car[filter.facet] <= filter.values[1]
        );
      }
    });
